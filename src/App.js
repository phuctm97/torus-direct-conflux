...
class App extends React.Component {
  ...
  login = async (e) => {
    ...
    try {
      ...
      this.cfx = await Conflux.create({
        url: "https://test.confluxrpc.org/v2",
        logger: console,
      });

      const account = this.cfx.wallet.addPrivateKey(
        `0x${loginDetails.privateKey}`
      );
      const balance = Drip(await this.cfx.getBalance(account.address)).toCFX();

      this.setState({
        consoleText: [
          `Logged in as "${account.address}"`,
  ...
  sendTransaction = async (e) => {
    e.preventDefault();
    const { cfxTransactionAmount, cfxTransactionTo, cfxAccount } = this.state;

    const hash = await this.cfx.sendTransaction({
      from: cfxAccount,
      to: cfxTransactionTo,
      value: Drip.fromCFX(cfxTransactionAmount),
    });
    this.setState({ consoleText: `Sent transaction ${hash}` });
  };
      ...
      <div className="App">
        ...
        <div id="app">
          ...
          <p>
            ...
            here only work with the client id's specified in example. Please
            don't edit them.
          </p>
          <div id="console">
            <p></p>
        ...
        {cfxAccount && (
          <div>
            Logged in as {cfxAccount.address}
            <p>Current balance: {cfxBalance} CFX</p>
            ...
            <form onSubmit={this.sendTransaction}>
              ...
              <input
                id="to"
                type="text"
                value={cfxTransactionTo}
                onChange={(e) =>
                  this.setState({ cfxTransactionTo: e.target.value })
                }
              />
              ...
              <input
                id="amount"
                type="number"
                value={cfxTransactionAmount}
                onChange={(e) =>
                  this.setState({
                    cfxTransactionAmount: parseFloat(e.target.value),
                  })
                }
              />
              <input type="submit" />
